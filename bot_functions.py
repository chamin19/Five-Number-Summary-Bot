...
def mode(temp): #make og
    most = max(list(map(temp.data_list.count, temp.data_list)))
    most_freq_list = list(set(filter(lambda x: temp.data_list.count(x) == most, temp.data_list)))
    output = [str(num) for num in most_freq_list]
    if len(output) == len(temp.data_list):
        return "None"
    return output
...
def maximum_fence(temp):
    ...
    if temp.data_list[-1] <= upper_fence(temp):
        ...
        for i in range(len(temp.data_list)-1,-1,-1):
            if temp.data_list[i] > upper_fence(temp):
                index = i
>>>>>>> f723844cb1ad9e4ec24bb1fdebac1bb0cde4960f
        return temp.data_list[index-1]